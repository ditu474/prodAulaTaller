...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  LoginBloc({@required loginWithCredentials,
  ...
  Stream<LoginState> _mapLoginWithCredentialsState({
    ...
    var credentials = LoginCredentialsInput.createFromStrings(email:email,password:password);
    if(credentials.isLeft()){
      ...
      var response = await _loginWithCredentials.execute(credentials.getOrElse(null));
      if(response.isRight()){
  ...
  Stream<LoginState> _mapFacebookButtonPressedState () async*{
    final response = await _openDynamicLink.execute("https://www.facebook.com/aulatallerpcjic/");
    if(response.isLeft()){
      yield state.copyWith(
        error:"No se logró abrir facebook",
        linkError: true);
    }
    else{
  ...
  Stream<LoginState> _mapInstagramButtonPressedState () async*{
    final response = await _openDynamicLink.execute("https://www.instagram.com/aulatallerpoli/");
    if(response.isLeft()){
      yield state.copyWith(
        error:"No se logró abrir instagram",
        linkError: true);
    }
    else{
      yield state.copyWith(linkError:false);
